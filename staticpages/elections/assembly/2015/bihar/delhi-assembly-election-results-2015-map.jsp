...
$.ajax({type: "GET",url: feedURL,dataType: "jsonp", cache: "true",crossDomain: true,jsonpCallback: 'e'+consID,success: function (data)
	...
	var tr;
  tr = "<tr><td colspan = '4' style='position:relative'><div class='p_head'><a href ='http://ittemp.intoday.in/elections/assembly/2015/delhi/delhi-assembly-election-results-2015-constituency-wise-map.jsp?consid="+consID+"&amp;cname="+data.loksabha.cName.toLowerCase().replace('-','').replace(' ','-').replace('.','').replace('-  (sc)','')+"' target = '_blank'>"+data.loksabha.cName+"</a></div><div style='position: absolute;right:10px;top: 8px;'><img src='https://itgd-mum-dev-static.s3.amazonaws.com/media/others/mediaintoday/indiatoday/election_delhi_2015/live-poll/close.gif' style = 'cursor:pointer;' onclick = 'tabclose();'/></div></td></tr>";
	var itemsArray = data.loksabha.items; 
	  for(var i = 0;i<itemsArray.length;i++)
	  {
