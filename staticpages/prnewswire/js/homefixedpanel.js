		...
		if(windoworientation==90) {
			...
			var toplocation = $(window).scrollTop();			
				...
				if(toplocation<=(toppanel+right_ht-window_ht)) {
					...
					$('#rightpanel').css({position:'absolute',top:newheightad,right:'0px'});
						if(windoworientation == -90) {
							$('#leftpart').css({position:'absolute',top:'1033px',left:'0px'});
						}
				}
				...
				if(toplocation>=(toppanel+right_ht-window_ht)) {
					...
					if(windoworientation == -90) {
						$('#leftpart').css({position:'fixed',top:'-990px', left: right_set});
					}
				}
...
//window.onscroll = function() { alert("Scrolled"); };
 ...
 $(document).ready(function(){
	...
	window.addEventListener('scroll', function() {
				...
				if(toplocation<=(toppanel+right_ht-window_ht)) {
					$('#rightpanel').css({position:'absolute',top:'580px',right:'20px'});
...
//
//        $.getScript("http://media2.intoday.in/indiatoday/speed-test/jquery.mCustomScrollbar.concat.min.js", function () {
//             $('.l_cnt_t').mCustomScrollbar({
//                   horizontalScroll: false,
//            autoDraggerLength: true,
//            autoHideScrollbar: true,
