        ...
        <div class ="video-landing-header">
            <div class="row">
                ...
                <div class="col-md-8 video-header-left">
                    ...
                    if (!empty($videoids)) {
                        $hide_player="";
                        $newimageds = '<div class="video-slider-images">';
                        foreach ($videoids as $keys => $video_value) {
                            if($keys!=0)
                            {
                                $hide_player='hide-player';
                            }
                            ?> <div class="video <?php echo $hide_player;?>" id="video_<?php echo $video_value->video_id;?>"><?php
                            if (module_exists('itg_videogallery')) {
                                $vid = itg_videogallery_get_videoid($row['fid']);
                            }
                            if($video_value->dailymotion_thumb_url!="") {     
                              $newimageds.= '<img data-tag="video_'.$video_value->video_id.'" src="'.$video_value->dailymotion_thumb_url.'" height="66" width="88">';
                            } else {
