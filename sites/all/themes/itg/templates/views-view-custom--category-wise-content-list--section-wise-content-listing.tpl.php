  ...
  <?php else: ?>
    ...
    <div class="detail"><h3 title="<?php echo strip_tags($row['title']);?>"><?php
      if (function_exists('itg_common_get_smiley_title')) {
        ...
        print l(html_entity_decode(strip_tags(mb_strimwidth($row['title'], 0, 120, ".."))), "node/" . $row['nid']);
      }
      ?></h3>
        <?php if (strtolower($row['type']) == 'story'): ?>
        <p><?php if (isset($row['field_story_kicker_text'])) {
        print html_entity_decode(strip_tags($row['field_story_kicker_text']));
      } ?></p>
      <?php
      ...
      if (!empty($row['field_video_kicker'])) {
        print '<p>' . html_entity_decode(strip_tags($row['field_video_kicker'])) . '</p>';
      }
      ?>            </div>
  </div>
<?php endforeach; ?>