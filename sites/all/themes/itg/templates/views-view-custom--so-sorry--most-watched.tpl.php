...
<?php foreach ($rows as $key => $value) : ?>
  ...
  <figure>
    ...
    <?php else : ?>
      <a href="<?php print $base_url . '/' . "sosorry/ " . $value['nid'] . "" ?>" class="pic">
        <img width='170' height='127'  src='<?php print $base_url . "/" . drupal_get_path('theme', 'itg'); ?>/images/itg_image170x127.jpg' alt='' title='' />
      </a>
    <?php endif; ?>
      <figcaption><i class="fa fa-play-circle"></i> <?php print $value['field_video_duration']; ?></figcaption>
          ...
          <p  title="<?php print strip_tags($value['title']) ; ?>"> 
            <?php print l($value['title'], $base_url . '/' . "sosorry/" . $value['nid'] . "" , array('html' => TRUE)) ?>
          </p>
        </div>
        </li>
<?php endforeach; ?>
