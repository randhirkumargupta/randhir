...
<div class="video-ftp-div">
    ...
    foreach ($rows as $id => $row) {
      ...
      if (empty($row['sm_field_video_thumb_url'])) {
        $image_path = image_style_url("widget_very_small", drupal_get_path('theme', 'itg') . '/images/itg_image88x66.jpg');
      }
      ...
      if (VIDEO_PROPERTY != $row['sm_field_property']) {
        if ($_GET['sm_field_video_used'] == "") {
          ...
          $video_image = '<img  width="100" height="44" src="' . $image_path . '">';
          $file_size = number_format($row['sm_field_video_size'] / (1024 * 1024), 2);
          $video_data .= '<div class="ftp_video_radio"><div class="video-checkbox-wraper"><input id = "video_id_' . $row['sm_field_video_id'] . '" type="radio" name="video-form" class="form-radio" value="' . $video_value . '"/></div><div class="serch-image">' . $video_image . '</div><div class="show_video_id">' . $row['sm_field_video_id'] . '</div><div class="show_video_title">' . $row['label'] . '</div><div class="file_size">' . $file_size . 'MB</div><div class="file_size">' . $row['sm_field_video_type'] . '</div><div class="file_size_duration">' . $row['sm_field_video_duration'] . '</div><div class="file_size_date_time">' . $row['sm_field_video_date_time'] . '</div><div><a href="javascript:void(0)" data-video-id ="' . $row['sm_field_video_id'] . '" data-type-repo = '.$row['sm_field_video_type'].' class="play-video"><i class="fa fa-play-circle" aria-hidden="true"></i> Play</a></div></div>';
        }
      }
    }
