            ...
            <div class="content-view">
              ...
              <?php if (isset($node->field_content_sharing_mode)) { ?> 
                ...
                if (trim($node->field_content_sharing_mode[LANGUAGE_NONE][0]['value']) == 1) {
                  $latest_url = itg_mobile_get_latest_feed_byurl($node->field_service_fetch_link[LANGUAGE_NONE][0]['value']);
                  if (!empty($latest_url[0])) {
                    $path = file_create_url('public://');
                    $real_path = substr($path, 0, -1) . $latest_url[0]->xml_name;
