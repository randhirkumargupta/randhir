      ...
      if ($row['sm_field_video_used'] == 1) {
        if (VIDEO_PROPERTY == $row['sm_field_property']) {
          if (!in_array($row['sm_field_video_id'], $all_draft_video)) {
            $video_value = $row['sm_field_video_id'] . '#' . $row['sm_field_video_size'] . '#' . $row['label'] . '#' . $row['sm_field_video_thumb_url'] . '#' . $row['sm_field_video_duration'] . '#' . $row['sm_field_property'] . '#other' . '#1#INTERNAL#' . $row['sm_field_all_xml_content'];
