...
(function ($) {
  Drupal.behaviors.itg_akamai_block_refresh = {
    attach: function (context) {
      $.each(Drupal.settings.itg_akamai_block_refresh.settings, function (key, settings) {
      ...
      function setBlockRefresh(element, element_content, auto, manual, init, timer, arguments, block, delta, panels) {
        ...
        query = '';
        if (arguments) {
          $.each(Drupal.settings.itg_akamai_block_refresh.args, function (index, arg) {
            args += '/' + arg;
          });
          query = Drupal.settings.itg_akamai_block_refresh.query;
        }
        var path = Drupal.settings.basePath + Drupal.settings.pathPrefix + 'itg_akamai_block_refresh/' + block + '/' + delta + args + query;
