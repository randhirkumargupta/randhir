...
itg_db_migrate_related_content_photo();

function itg_db_migrate_related_content_photo() {
  //Code for related content
  //code for XML read
  $xml_dir = 'related-photo/';
  ...
  foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path_xml)) as $filename) {
    ...
    foreach ($xml_data->relatedcontents as $xml) {
      ...
      if (!empty($content_id)) {
        ...
        $related_content_val = '';
        $content_revision_id = get_node_revision_id_photo($content_id);
        ...
        if (!empty($related_content_val)) {
          ...
          itg_data_insert_in_field_photo($content_id, $content_revision_id, 'field_common_related_content', 'photogallery', 'node', $related_content_insert_val);
          itg_data_insert_in_field_photo($content_id, $content_revision_id, 'field_cm_related_content_detail', 'photogallery', 'node', $related_content_insert_title_val);
 ...
 */
function get_node_revision_id_photo($nid) {
...
function itg_data_insert_in_field_photo($nid, $revision_id, $field_name, $bundle, $entity_type, $data) {
