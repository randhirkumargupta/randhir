...
photo_field_update_category_map();

function photo_field_update_category_map() {
  $path_xml = 'sites/default/files/migrate/xml_file/photo-2017/';
          ...
          foreach ($node->field_story_category['und'] as $photo_cat) {
            foreach ($photo_array_val as $key => $photo_array) {
              if ($photo_cat['tid'] == $photo_array['photo_category_id']) {
                ...
                if (!empty($photo_array['section']) && !empty($photo_array['category_id'])) {
                 ...
                 if($photo_array['category_id'] != $photo_cat['tid']) {
                   ...
                   if(!empty($new_photo_sec) && !empty($new_photo_category)){
                      $new_photo_cat = array_merge($new_photo_sec, $new_photo_category);
                  }
      //    pr($new_photo_sec);   
        //  pr($new_photo_category);
        //  p($new_photo_cat[0]);
          // Primary category code.
          $p_catgory = (string) $xml->primary_category;
          $priy_cat = explode('#', $p_catgory);
          $original_primary_category = end($priy_cat);
          $primary_category = get_itg_destination_id('migrate_map_itgphoto', $original_primary_category);
          foreach ($photo_array_val as $key => $photo_array) {
              if ($primary_category == $photo_array['photo_category_id']) {
                if (!empty($photo_array['category_id'])) {
                  $primary_category_new = $photo_array['category_id'];
                }
              }
            }
  // p($primary_category_new);
...
function category_mapping_script() {
  $file_path = 'sites/default/files/migrate/xml_file/new_cat_map/';
  $file_name = 'ContentMapping-Complete-Master_KT_Category-TRAIL.csv';
  ...
  if (($handle = fopen($file_path . $file_name, "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
      ...
      $count++;
      for ($c = 0; $c < $num; $c++) {
        $value_photo[$count]['photo_category_id'] = (!empty($data[9] && is_numeric($data[9])) ? get_itg_destination_id('migrate_map_itgphoto', $data[9]) : 'NA');
        ...
        $value_photo[$count]['photo_orig_category_id'] = (!empty($data[9] && is_numeric($data[9])) ? $data[9] : 'NA');
...
function check_updated_node_photo($nid, $xml_id, $xml_file_name) {
  ...
  $query->condition('image_update', 'photo_final_category_50000', '=');
  $result = $query->execute()->fetchField();
...
function node_update_insert_status_update_photo($nid = NULL, $xml_id, $file_name) {
  db_insert('story_fields_update_script_check_photo')
      ->fields(array(
        ...
        'nid' => $nid,
        'xml_name' => $file_name,
        'image_update' => 'photo_final_category_50000',
      ))
      ->execute();
}
