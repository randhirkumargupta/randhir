...
function category_mapping_script() {
  ...
  $file_name = 'ContentMapping-Complete-Master_KT_Category.csv';
  ...
  if (($handle = fopen($file_path . $file_name, "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
      ...
      $count++;
      for ($c = 0; $c < $num; $c++) {
        $value_photo[$count]['video_category_id'] = (!empty($data[8] && is_numeric($data[8])) ? get_itg_destination_id('migrate_map_itgcategory', $data[8]) : 'NA');
        ...
        $value_photo[$count]['video_orig_category_id'] = (!empty($data[8] && is_numeric($data[8])) ? $data[8] : 'NA');
...
function check_updated_node_photo($nid, $xml_id, $xml_file_name) {
  ...
  $query->condition('image_update', 'photo_final_category_update', '=');
  $result = $query->execute()->fetchField();
...
function node_update_insert_status_update_photo($nid = NULL, $xml_id, $file_name) {
  db_insert('story_fields_update_script_check_photo')
      ->fields(array(
        ...
        'nid' => $nid,
        'xml_name' => $file_name,
        'image_update' => 'photo_final_category_update',
      ))
      ->execute();
}
