const res_version = 'indiatoday-2.3'; 
...
//const MEDIA_CACHE_PATH_NAME = '/assets/';
...
function openDatabaseAndReplayRequests() {
    var indexedDBOpenRequest = indexedDB.open('mobile-tak-offline-analytics', IDB_VERSION);
            ...
            caches.keys().then(function(cacheNames) {
                return Promise.all(
                    cacheNames.map(function(cacheName) {
                        ...
                        if (cacheName !== APP_CACHE_NAME.cache.name && cacheName !== STATIC_CACHE_NAME.cache.name) {
    ...
    if (e.request.method === 'GET') {
        ...
        if (SITE_CACHE_HOST_NAME.indexOf(url.hostname) !== -1) {
            if (e.request.url.indexOf(ROUTE_SITE_URL) !== -1 || e.request.url.indexOf(ROUTE_SITE_URL + '/') !== -1) {
                if (e.request.url.indexOf(PAGENOT_FOUND_URL) !== -1) {} else if (e.request.url.indexOf(OFFLINE_URL) !== -1) {
                    e.respondWith(
